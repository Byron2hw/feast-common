...
public class WebClientRequestExceptionCommonHandler extends AbstractExceptionCommonHandler {
    ...
    public Mono<ServerResponse> handlerException(ServerWebExchange exchange, Exception e) {
        ...
        return super.monoServerResponse(HttpStatus.BAD_REQUEST.value(), Result.fail(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST.series().name(), Optional.ofNullable(e.getMessage()).orElse(HttpStatus.BAD_REQUEST.getReasonPhrase())));
    }
